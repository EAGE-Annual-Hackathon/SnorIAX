...
class AttentionExpandingBlock(nn.Module):
    """Attention Expanding block
    ...
    """
    def __init__(self, input_channels, use_dropout=False, use_bn=True):
        super(AttentionExpandingBlock, self).__init__()
        ...
        self.Att   = AttentionBlock(F_g= input_channels // 2, F_l= input_channels // 2, n_coefficients= input_channels // 4)
        if use_bn:
            self.batchnorm = nn.BatchNorm2d(input_channels // 2, momentum=0.8)
        self.use_bn = use_bn
    ...
    def forward(self, x, skip_con_x):
        ...
        a = self.Att(gate=x, skip_connection=skip_con_x) # Attention
...
class AttentionUNet(nn.Module):
    ...
    def __init__(self, input_channels=1, output_channels=1, hidden_channels=64, levels=2):
        ...
        for level in range(levels):
            self.expand.append(AttentionExpandingBlock(hidden_channels * (2 ** (levels - level))))
    ...
    def forward(self, x):
        ...
        for level in range(self.levels):
            ...
            xenc.append(x)
        for level in range(self.levels):
            x, at = self.expand[level](x, xenc[self.levels - level - 1])
            attn.append(at)
...
class AttentionBlock(nn.Module):
    """Attention block with learnable parameters
        from  https://github.com/sfczekalski/attention_unet/blob/master/model.py
    """
